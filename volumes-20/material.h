...
{
    public:
        __device__ diffuse_light(e_texture* a) : emit(a) {}
        __device__ virtual bool scatter(const ray& r_in, const hit_record& rec, vec3& attenuation, ray& scattered, curandState* rand_state) const { return false; }
        __device__ virtual vec3 emitted(float u, float v, const vec3& p) const { return emit->value(u, v, p); }
        e_texture* emit;
};
...
{
    public:
        __device__ isotropic(e_texture* a) : albedo(a) {}
        __device__ virtual bool scatter(const ray& r_in, const hit_record& rec, vec3& attenuation, ray& scattered, curandState* rand_state) const
        {
            scattered = ray(rec.p, random_in_unit_sphere(rand_state));
            attenuation = albedo->value(rec.u, rec.v, rec.p);
            return true;
        }
        e_texture* albedo;
